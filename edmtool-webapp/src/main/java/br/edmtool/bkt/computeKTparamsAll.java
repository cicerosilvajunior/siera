...
public class computeKTparamsAll {
	...
	//retornar a licao associada ao skill
	public String lessons_[] = new String[27600];//Number of Lessons
	...
	public void read_in_data(StreamTokenizer st_) {
		...
		try {
			...
			while (tt != StreamTokenizer.TT_EOF) {
				...
				//TODO remover caso o teste de retorno da li��o nao funcione
				lessons_[actnum] = st_.sval;
				...
				if (!skill_[actnum - 1].equals(prevskill) || !students_[actnum - 1].equals(prevStudent)) {
					prevskill = skill_[actnum - 1];
					prevStudent = students_[actnum - 1];
...
//					prevskill = skill_[actnum - 1];
			...
			e.printStackTrace();
			logger.error("Activity number" + actnum, e);
		}
	...
	public String[] fit_skill_model(int curskill) {
		double SSR = 0.0;
		...
		String[] bestParams = new String[7];
		bestParams[0] = students_[startact];
		bestParams[1] = lessons_[startact];
		bestParams[2] = skill_[startact];
		bestParams[3] = String.valueOf(bestLzero);
		bestParams[4] = String.valueOf(bestG);
		bestParams[5] = String.valueOf(bestS);
		bestParams[6] = String.valueOf(besttrans);
				
		return bestParams;
	}

	public List<String[]> computelzerot(String infile_) {
		StreamTokenizer st_ = create_tokenizer(infile_);

		read_in_data(st_);
		
		String[] bestParams = null;
		List<String[]> listStudentsBKT = new ArrayList<String[]>();
		//TODO adicionar header para arquivo CSV (listStudentsBKT.add(new String[] { "ID", "Name", "Age", "Country" });
		
		logger.debug("Students list size " + listStudentsBKT.size());
		
		logger.info("student\tlesson\tskill\tL0\tG\tS\tT\t");
		for (int curskill = 0; curskill <= skillnum; curskill++) {
			bestParams = fit_skill_model(curskill);
			logger.info("Best params " + Arrays.toString(bestParams));
			listStudentsBKT.add(bestParams);
			bestParams = null;
		}
		
		logger.debug("Students list size " + listStudentsBKT.size());
		
		return listStudentsBKT;		
	}
